      ...
      <template v-if="multiple">
        <span v-if="collapseTags && currentLabels.length">
          <el-tag
            ...
            disable-transitions>
            <span class="el-select__tags-text" v-if="showAllLevels">{{ currentLabels[0].join(separator) }}</span>
            <span class="el-select__tags-text" v-else>{{ currentLabels[0][currentLabels[0].length - 1] }}</span>
          </el-tag>
          <el-tag
            v-if="currentLabels.length > 1"
            :closable="false"
            :size="collapseTagSize"
            type="info"
            disable-transitions>
        ...
        <transition-group @after-leave="resetInputHeight" v-if="!collapseTags">
          <el-tag
            ...
            disable-transitions>
            <span class="el-select__tags-text" v-if="showAllLevels">{{ item.join(separator) }}</span>
            <span class="el-select__tags-text" v-else>{{ item[item.length - 1] }}</span>
          </el-tag>
        </transition-group>
        <el-input
          ref="multiInput"
